...
if(isset($_POST['acceder'])){
  ...
  try{
    ...
    //final test pour avoir accès il faut tester que son compte est validé par le CA
            $stmt = $pdo->prepare('SELECT id_user, nom, prenom, email, password FROM adherent WHERE email = :email AND valide=0');
            $stmt->execute(array(
                ':email' => $email
            ));
            ...
            else
            {
                if(md5($password) == $data['password'])
                {
                    ...
                    header('Location: account.php?'.$_SESSION['id']);
                    exit;
                }
                else
                {
  ...
  <body class="text-center">
    <form class="form-reserv" action="" method="POST">
      ...
      <input type="submit" name="acceder" class="btn btn-lg btn-danger btn-block" value="Accéder"> <br> 
       <a href="index.html" class="btn btn-success btn-lg btn-block"> retour à l'accueil</a>    
